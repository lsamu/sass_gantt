<template>
  <div class="gantt" v-loading="localLoading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading"
    element-loading-background="rgba(0, 0, 0, 0.8)">
    <div class="container">
      <div class="field-table-container" :style="{
        marginLeft: fieldTableOpen ? '0px' : -fieldTableWidth + 'px'
      }">
        <el-table :data="tableData" class="field-table" rowKey="id" @header-dragend="handleDragend"
          :defaultExpandAll="true" :border="true" :highlightCurrentRow="false" :spanMethod="spanMethod"
          :treeProps="{ children: 'children', hasChildren: 'hasChildren' }" :class="timeType" :row-class-name="setRowName"
          @cell-mouse-enter="fieldTrEnter" @cell-mouse-leave="fieldTrLeave" @expand-change="expendChange"
          style="width: 100%">
          <el-table-column v-for="(field, fi) in columns" :prop="field.prop" :property="field.prop" :key="fi"
            :label="field.label" fixed="left" :resizable="fi !== 0" :width="field.width" class-name="is-field expend">
            <template slot-scope="scope">
              <CellTime v-if="field.slot === 'time'" :row="scope.row" @change="time => {
                  scope.row[field.prop] = time;
                }
                " :field="field"></CellTime>
              <CellProcess v-else-if="field.slot === 'process'" :row="scope.row" :field="field"></CellProcess>

              <span class="expend-container" v-else-if="field.slot === 'text'">
                <CellInput :row="scope.row" :field="field"></CellInput>
                <el-popover placement="right" width="200" class="extend-inner" trigger="click">
                  <div class="menu">
                    <p>上一级</p>
                    <p>下一级</p>
                    <p>升级</p>
                  ...
                  <span slot="reference" class="title-cell">
              ...
              <CellUserSearch v-else-if="field.slot === 'user'" :row="scope.row" :field="field"></CellUserSearch>
              <span v-else :title="scope.row[field.prop]">{{ scope.row[field.prop] }}</span>
            </template>
          </el-table-column>
        </el-table>
        <div class="opener" @click="fieldTableOpen = !fieldTableOpen" :style="{ left: fieldTableWidth + 'px' }">
          <div class="inner">
            <span class="block">
              <i class="el-icon-arrow-left" :class="{ close: !fieldTableOpen }"></i>
            </span>
          </div>
        </div>
      </div>

      <div class="gantt-table-container" :style="{
        paddingLeft: fieldTableOpen ? fieldTableWidth + 'px' : '0px'
      }">
        <div class="warpper">
          <div class="warpper-inner">
            <table class="gantt-table" :style="{ width: tableWidth + 'px' }" cellspacing="0" cellpadding="0">
              <colgroup>
                <col v-for="(item, index) in colgroups" :key="index + '.' + item" :width="item" />
              </colgroup>
              <thead>
                <tr v-for="(tr, index) in thColumns" :key="index">
                  <th v-for="(th, thIndex) in tr" :key="thIndex" rowspan="1" :title="getTitle(th, index)"
                    :colspan="th.offset" :style="index === 2
                        ? {
                          height: '0px',
                          border: 'none'
                        }
                        : {}
                      ">
                    <span class="th-span" v-if="index !== 2" :style="index === 0
                        ? {
                          width:
                            th.offset *
                            (timeType === 'week'
    ...
    today:
      showToday &&
        ...
        : false
              ...
              <tbody>
                <tr v-for="(item, index) in deepData" @mouseenter.stop="fieldTrEnter(item)"
                  @mouseleave.stop="fieldTrLeave(item)" :key="index" :class="[
                    { collapse: checkCollapse(item) },
                    item.rowKey,
                    'gantt-row'
                  ]">
                  <td v-for="(td, tdIndex) in getTds(item, thColumns)" :key="tdIndex" style="height: 48px;"
                    :colspan="getSpan(item, td)[1]">
                    <GanttBar v-if="timeType === 'day' &&
                      td.prop === dayjs(item.startTime).format('YYYY-MM-DD')
                      " :item="item" :barHeight="barHeight" :left="getBarLeft(item)" :process="item.percent"
                      :scrollX="scrollX" :offsetX="fieldTableOpen ? fieldTableWidth : 0" :width="getBarWidth(item)">
                    </GanttBar>

                    <GanttBar v-if="timeType === 'week' &&
                      td.prop ===
                      dayjs(item.startTime)
                        .startOf('week')
                        .format('YYYY-MM-DD')
                      " :item="item" :left="getBarLeft(item)" :process="item.percent" :barHeight="barHeight"
                      :scrollX="scrollX" :offsetX="fieldTableOpen ? fieldTableWidth : 0" :width="getBarWidth(item)">
                    </GanttBar>
                    <GanttBar v-if="timeType === 'month' &&
                      td.prop === dayjs(item.startTime).format('YYYY-MM')
                      " :item="item" :left="getBarLeft(item)" :process="item.percent" :barHeight="barHeight"
                      :scrollX="scrollX" :offsetX="fieldTableOpen ? fieldTableWidth : 0" :width="getBarWidth(item)">
                    </GanttBar>
                    <GanttBar v-if="timeType === 'quarter' &&
                      td.prop.indexOf(
            ...
            <TodayColumnLine v-if="showToday" :offsetX="getOffsetX(dayjs())" :cellWidth="localCellWidth"
    ...
    <div class="div-container">
      <div class="field-container" :style="{
        ...
        <FieldTableHead :columns="localFieldColumns" @sizeChange="headSizeChange"></FieldTableHead>

        <div class="body">
          <draggable v-model="localData" :group="{ name: 'root' }">
            <FieldTableRow v-for="tr in localData" :key="tr.id" :item="tr" :columns="localFieldColumns"></FieldTableRow>
          </draggable>
        </div>
        <div class="opener" @click="fieldTableOpen = !fieldTableOpen" :style="{ left: fieldTableWidth + 'px' }">
          <div class="inner">
